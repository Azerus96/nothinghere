javascript:(function(){
  window.__avtrxMasterV9 = false;
  if (window.__avtrxHunterV11) {
    alert('Hunter v11 уже активен!');
    return;
  }
  window.__avtrxHunterV11 = true;
  ...
  // Постоянный опрос радара игроков
  ...
  // Перехват сетевых вызовов участников (с сохранением ников и ставок)
  ...
  function recordRound(m) {
    ...
    var rData = {
      ...
      whales_in_round: currentWhales.length ? currentWhales : null
    };

    rounds.push(rData);
    peakPlayers = 0;
    // Сохраняем китов для следующего раунда, если новый список еще не пришел
  ...
  // HUD
  var old = document.getElementById('avtrx-v11-hud');
  if (old) old.remove();

  var hud = document.createElement('div');
  hud.id = 'avtrx-v11-hud';
  ...
  hud.innerHTML = '<div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:6px;">' +
                  '<span style="color:#38bdf8;font-weight:bold;">✈ HUNTER v11</span>' +
                  '<span id="v11-status" style="color:#facc15;font-weight:bold;font-size:13px;">Слежу за китами...</span>' +
                  '<b id="v11-cnt" style="color:#4ade80;font-size:12px;">0 R</b>' +
                  '</div>' +
                  '<div id="v11-stats" style="font-size:10px;color:#94a3b8;margin-bottom:8px;line-height:1.4;">' +
                  '< 1.10: 0% | < 1.50: 0% | > 10x: 0%' +
                  '</div>' +
                  '<div style="display:flex;gap:6px;">' +
                  '<button id="v11-save" style="flex:1;background:#16a34a;color:#fff;border:none;padding:6px;border-radius:4px;font-weight:bold;cursor:pointer;">💾 СКАЧАТЬ ДАТАСЕТ</button>' +
                  '<button id="v11-stat-btn" style="background:#0284c7;color:#fff;border:none;padding:6px 10px;border-radius:4px;cursor:pointer;">📊 ИНФО</button>' +
                  '<button id="v11-clr" style="background:#475569;color:#fff;border:none;padding:6px 8px;border-radius:4px;cursor:pointer;">🧹</button>' +
                  '</div>';
  document.body.appendChild(hud);

  function updateHUD(lastM, flightSec, players){
    var cEl = document.getElementById('v11-cnt');
    var sEl = document.getElementById('v11-status');
    var stEl = document.getElementById('v11-stats');
    ...
    if (stEl && buckets.total > 0) {
      ...
      stEl.innerHTML = '< 1.10: <b>' + p11 + '%</b> | < 1.50: <b>' + p15 + '%</b> | > 10x: <b>' + p10 + '%</b>';
    }
  }

  document.getElementById('v11-stat-btn').onclick = function(){
    if (!rounds.length) return alert('Пока нет данных!');
    alert('📊 СТАТИСТИКА:\n\n' +
          ' • Раундов: ' + rounds.length + '\n' +
          ' • Средний онлайн: ' + Math.round(rounds.reduce(function(a,b){return a+b.players_online;},0)/rounds.length) + ' чел.\n' +
          ...
          'Текущая серия без 10x: ' + streakCount + ' раундов');
  };

  document.getElementById('v11-save').onclick = function(){
    ...
    a.download = 'aviatrix_hunter_v11_' + Date.now() + '.json';
    a.click();
  };

  document.getElementById('v11-clr').onclick = function(){
    rounds = [];
